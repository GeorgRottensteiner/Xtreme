...
BOOL IMF_Init(void)
{
	...
	if(!(imf_mh=(IMFHEADER*)MikMod_malloc(sizeof(IMFHEADER)))) return 0;
...
#define IMF_SMPINCR 64
	...
	_mm_read_string(imf_mh->songname,32,modreader);
	imf_mh->ordnum=_mm_read_I_UWORD(modreader);
	imf_mh->patnum=_mm_read_I_UWORD(modreader);
	imf_mh->insnum=_mm_read_I_UWORD(modreader);
	imf_mh->flags =_mm_read_I_UWORD(modreader);
	_mm_fseek(modreader,8,SEEK_CUR);
	imf_mh->initspeed =_mm_read_UBYTE(modreader);
	imf_mh->inittempo =_mm_read_UBYTE(modreader);
	imf_mh->mastervol =_mm_read_UBYTE(modreader);
	imf_mh->mastermult=_mm_read_UBYTE(modreader);
	...
	of.songname=DupStr(imf_mh->songname,31,1);
	of.modtype=_strdup(IMF_Version);
	of.numpat=imf_mh->patnum;
	of.numins=imf_mh->insnum;
	of.reppos=0;
	of.initspeed=imf_mh->initspeed;
	of.inittempo=imf_mh->inittempo;
	of.initvolume=imf_mh->mastervol<<1;
	of.flags |= UF_INST | UF_ARPMEM | UF_PANNING;
	if(imf_mh->flags&1) of.flags |= UF_LINEAR;
	of.bpmlimit=32;
	...
	/* read order list */
	_mm_read_UBYTES(imf_mh->orders,256,modreader);
	if(_mm_eof(modreader)) {
		_mm_errno = MMERR_LOADING_HEADER;
		return 0;
	}
	...
	of.numpos=0;
	for(t=0;t<imf_mh->ordnum;t++)
		if(imf_mh->orders[t]!=0xff) of.numpos++;
	if(!AllocPositions(of.numpos)) return 0;
	for(t=u=0;t<imf_mh->ordnum;t++)
		if(imf_mh->orders[t]!=0xff) of.positions[u++]=imf_mh->orders[t];
