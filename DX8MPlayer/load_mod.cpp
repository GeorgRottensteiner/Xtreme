...
typedef struct tMODHeader {
	CHAR songname[21];			/* the songname.. 20 in module, 21 in memory */
	tMODSampInfo samples[31];		/* all sampleinfo */
...
typedef struct tMODType {
	CHAR id[5];
	UBYTE channels;
	CHAR *name;
} tMODType;

typedef struct tMODNote {
	UBYTE a, b, c, d;
} tMODNote;
...
static tMODHeader *mod_mh = NULL;
static tMODNote *mod_patbuf = NULL;
static int tMODType, trekker;

/*========== Loader code */

/* given the module ID, determine the number of channels and the tracker
   description ; also alters tMODType */
static BOOL MOD_CheckType(UBYTE *id, UBYTE *numchn, CHAR **descr)
{
	...
	if (((!memcmp(id + 2, "CH", 2)) || (!memcmp(id + 2, "CN", 2)))
		...
		if (id[3] == 'H') {
			*descr = fasttracker;
			tMODType = 2;		/* this can also be Imago Orpheus */
		} else {
			*descr = taketracker;
			tMODType = 1;
		}
...
{
	if (!(mod_mh = (tMODHeader *)MikMod_malloc(sizeof(tMODHeader))))
		return 0;
	return 1;
}
...
static UBYTE ConvertNote(tMODNote *n, UBYTE lasteffect)
{
	...
	if (instrument) {
		...
		if ((instrument > 31) || (!mod_mh->samples[instrument - 1].length)) {
			UniPTEffect(0xc, 0);
			if (effect == 0xc)
				effect = effdat = 0;
		} else {
...
static UBYTE *ConvertTrack(tMODNote *n, int numchn)
{
	int t;
...
static BOOL MOD_ML_LoadPatterns(void)
{
	int t, s, tracks = 0;
	...
	if (!(mod_patbuf = (tMODNote *)MikMod_calloc(64U * of.numchn, sizeof(tMODNote))))
		return 0;
	...
	if (trekker && of.numchn == 8) {
		...
		for (t = 0; t < of.numpat; t++) {
			for (s = 0; s < (64U * 4); s++) {
				mod_patbuf[s].a = _mm_read_UBYTE(modreader);
				mod_patbuf[s].b = _mm_read_UBYTE(modreader);
				mod_patbuf[s].c = _mm_read_UBYTE(modreader);
				mod_patbuf[s].d = _mm_read_UBYTE(modreader);
			}
			for (s = 0; s < 4; s++)
				if (!(of.tracks[tracks++] = ConvertTrack(mod_patbuf + s, 4)))
					return 0;
			for (s = 0; s < (64U * 4); s++) {
				mod_patbuf[s].a = _mm_read_UBYTE(modreader);
				mod_patbuf[s].b = _mm_read_UBYTE(modreader);
		...
		for (t = 0; t < of.numpat; t++) {
			...
			for (s = 0; s < (SLONG)(64U * of.numchn); s++) {
				mod_patbuf[s].a = _mm_read_UBYTE(modreader);
				mod_patbuf[s].b = _mm_read_UBYTE(modreader);
				mod_patbuf[s].c = _mm_read_UBYTE(modreader);
				mod_patbuf[s].d = _mm_read_UBYTE(modreader);
			...
			for (s = 0; s < of.numchn; s++)
				if (!(of.tracks[tracks++] = ConvertTrack(mod_patbuf + s, of.numchn)))
					return 0;
		}
	}
	return 1;
...
static BOOL MOD_Load(BOOL curious)
{
	...
	_mm_read_string((CHAR *)mod_mh->songname, 20, modreader);
	mod_mh->songname[20] = 0;		/* just in case */
	...
	mod_mh->songlength = _mm_read_UBYTE(modreader);

	/* this fixes mods which declare more than 128 positions. 
	 * eg: beatwave.mod */
	if (mod_mh->songlength > 128) { mod_mh->songlength = 128; }
	
	mod_mh->magic1 = _mm_read_UBYTE(modreader);
	_mm_read_UBYTES(mod_mh->positions, 128, modreader);
	_mm_read_UBYTES(mod_mh->magic2, 4, modreader);
	...
	of.initspeed = 6;
	of.inittempo = 125;
	if (!(MOD_CheckType(mod_mh->magic2, &of.numchn, &descr))) {
	...
	if (trekker && of.numchn == 8)
		for (t = 0; t < 128; t++)
			...
			if (mod_mh->positions[t] & 1) {
				of.numchn = 4;
				break;
			}
	if (trekker && of.numchn == 8)
		for (t = 0; t < 128; t++)
			mod_mh->positions[t] >>= 1;

	of.songname = DupStr(mod_mh->songname, 21, 1);
	of.numpos = mod_mh->songlength;
	of.reppos = 0;

	/* Count the number of patterns */
	of.numpat = 0;
	for (t = 0; t < of.numpos; t++)
		if (mod_mh->positions[t] > of.numpat)
			of.numpat = mod_mh->positions[t];
	...
	for (t = of.numpos; t < 128; t++)
		if (mod_mh->positions[t] >= 0x80)
			scan = 0;
	if (scan)
		for (t = of.numpos; t < 128; t++) {
			if (mod_mh->positions[t] > of.numpat)
				of.numpat = mod_mh->positions[t];
			if ((curious) && (mod_mh->positions[t]))
				of.numpos = t + 1;
		}
	of.numpat++;
	...
	if (!AllocPositions(of.numpos))
		return 0;
	for (t = 0; t < of.numpos; t++)
		of.positions[t] = mod_mh->positions[t];
    ...
    //dh::Log( "Sample %d name dup %x", t, q->samplename );
