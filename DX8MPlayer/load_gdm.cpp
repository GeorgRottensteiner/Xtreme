...
static GDgdm_mhEADER *gdm_mh=NULL;	/* pointer to GDM header */
...
BOOL GDM_Init(void)
{
	...
	if (!(gdm_mh=(GDgdm_mhEADER*)MikMod_malloc(sizeof(GDgdm_mhEADER)))) return 0;
...
BOOL GDM_Load(BOOL curious)
{
	...
	/* read header */
	_mm_read_string(gdm_mh->id1,4,modreader);
	_mm_read_string(gdm_mh->songname,32,modreader);
	_mm_read_string(gdm_mh->author,32,modreader);
	_mm_read_string(gdm_mh->eofmarker,3,modreader);
	_mm_read_string(gdm_mh->id2,4,modreader);

	gdm_mh->majorver=_mm_read_UBYTE(modreader);
	gdm_mh->minorver=_mm_read_UBYTE(modreader);
	gdm_mh->trackerid=_mm_read_I_UWORD(modreader);
	gdm_mh->t_majorver=_mm_read_UBYTE(modreader);
	gdm_mh->t_minorver=_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(gdm_mh->pantable,32,modreader);
	gdm_mh->mastervol=_mm_read_UBYTE(modreader);
	gdm_mh->mastertempo=_mm_read_UBYTE(modreader);
	gdm_mh->masterbpm=_mm_read_UBYTE(modreader);
	gdm_mh->flags=_mm_read_I_UWORD(modreader);

	gdm_mh->orderloc=_mm_read_I_ULONG(modreader);
	gdm_mh->ordernum=_mm_read_UBYTE(modreader);
	gdm_mh->patternloc=_mm_read_I_ULONG(modreader);
	gdm_mh->patternnum=_mm_read_UBYTE(modreader);
	gdm_mh->sagdm_mhead=_mm_read_I_ULONG(modreader);
	gdm_mh->samdata=_mm_read_I_ULONG(modreader);
	gdm_mh->samnum=_mm_read_UBYTE(modreader);
	gdm_mh->messageloc=_mm_read_I_ULONG(modreader);
	gdm_mh->messagelen=_mm_read_I_ULONG(modreader);
	gdm_mh->scrollyloc=_mm_read_I_ULONG(modreader);
	gdm_mh->scrollylen=_mm_read_I_UWORD(modreader);
	gdm_mh->graphicloc=_mm_read_I_ULONG(modreader);
	gdm_mh->graphiclen=_mm_read_I_UWORD(modreader);
	...
	/* any orders? */
	if(gdm_mh->ordernum==255) {
		_mm_errno=MMERR_LOADING_PATTERN;
		return 0;
	}

	/* now we fill */
	of.modtype=_strdup(GDM_Version);
	of.modtype[18]=gdm_mh->majorver+'0';
	of.modtype[20]=gdm_mh->minorver/10+'0';
	of.modtype[21]=gdm_mh->minorver%10+'0';
	of.songname=DupStr(gdm_mh->songname,32,0);
	of.numpat=gdm_mh->patternnum+1;
	of.reppos=0;
	of.numins=of.numsmp=gdm_mh->samnum+1;
	of.initspeed=gdm_mh->mastertempo;
	of.inittempo=gdm_mh->masterbpm;
	of.initvolume=gdm_mh->mastervol<<1;
	...
	/* read the order data */
	if (!AllocPositions(gdm_mh->ordernum+1)) {
		_mm_errno=MMERR_OUT_OF_MEMORY;
		return 0;
	}

	_mm_fseek(modreader,gdm_mh->orderloc,SEEK_SET);
	for (i=0;i<gdm_mh->ordernum+1;i++)
	...
	q=of.samples;
	position=gdm_mh->samdata;

	/* seek to instrument position */
	_mm_fseek(modreader,gdm_mh->sagdm_mhead,SEEK_SET);
	...
	/* set the panning */
	for (i=x=0;i<32;i++) {
		of.panning[i]=gdm_mh->pantable[i];
		if (!of.panning[i])
	...
	/* jump to patterns */
	_mm_fseek(modreader,gdm_mh->patternloc,SEEK_SET);
