...
static S3MHEADER *s3m_mh      = NULL;
static UWORD     *s3m_paraptr = NULL; /* parapointer array (see S3M docs) */
...
BOOL S3M_Init(void)
{
	...
	if(!(s3m_mh=(S3MHEADER*)MikMod_malloc(sizeof(S3MHEADER)))) return 0;
...
static BOOL S3M_GetNumChannels(void)
{
	...
	while(row<64) {
		...
		if(flag) {
			ch=flag&31;
			if(s3m_mh->channels[ch]<32) remap[ch] = 0;
...
BOOL S3M_Load(BOOL curious)
{
	...
	_mm_read_string(s3m_mh->songname,28,modreader);
	s3m_mh->t1a         =_mm_read_UBYTE(modreader);
	s3m_mh->type        =_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(s3m_mh->unused1,2,modreader);
	s3m_mh->ordnum      =_mm_read_I_UWORD(modreader);
	s3m_mh->insnum      =_mm_read_I_UWORD(modreader);
	s3m_mh->patnum      =_mm_read_I_UWORD(modreader);
	s3m_mh->flags       =_mm_read_I_UWORD(modreader);
	s3m_mh->tracker     =_mm_read_I_UWORD(modreader);
	s3m_mh->fileformat  =_mm_read_I_UWORD(modreader);
	_mm_read_string(s3m_mh->scrm,4,modreader);
	s3m_mh->mastervol   =_mm_read_UBYTE(modreader);
	s3m_mh->initspeed   =_mm_read_UBYTE(modreader);
	s3m_mh->inittempo   =_mm_read_UBYTE(modreader);
	s3m_mh->mastermult  =_mm_read_UBYTE(modreader);
	s3m_mh->ultraclick  =_mm_read_UBYTE(modreader);
	s3m_mh->pantable    =_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(s3m_mh->unused2,8,modreader);
	s3m_mh->special     =_mm_read_I_UWORD(modreader);
	_mm_read_UBYTES(s3m_mh->channels,32,modreader);
	...
	tracker=s3m_mh->tracker>>12;
	if((!tracker)||(tracker>=NUMTRACKERS))
		tracker=NUMTRACKERS-1; /* unknown tracker */
	else {
		if(s3m_mh->tracker>=0x3217)
			tracker=NUMTRACKERS+1; /* IT 2.14p4 */
		else if(s3m_mh->tracker>=0x3216)
			tracker=NUMTRACKERS; /* IT 2.14p3 */
		else tracker--;
	}
	of.modtype = _strdup(S3M_Version[tracker]);
	if(tracker<NUMTRACKERS) {
		of.modtype[numeric[tracker]] = ((s3m_mh->tracker>>8) &0xf)+'0';
		of.modtype[numeric[tracker]+2] = ((s3m_mh->tracker>>4)&0xf)+'0';
		of.modtype[numeric[tracker]+3] = ((s3m_mh->tracker)&0xf)+'0';
	}
	/* set module variables */
	of.songname    = DupStr(s3m_mh->songname,28,0);
	of.numpat      = s3m_mh->patnum;
	of.reppos      = 0;
	of.numins      = of.numsmp = s3m_mh->insnum;
	of.initspeed   = s3m_mh->initspeed;
	of.inittempo   = s3m_mh->inittempo;
	of.initvolume  = s3m_mh->mastervol<<1;
	of.flags      |= UF_ARPMEM | UF_PANNING;
	if((s3m_mh->tracker==0x1300)||(s3m_mh->flags&64))
		of.flags|=UF_S3MSLIDES;
	of.bpmlimit    = 32;

	/* read the order data */
	if(!AllocPositions(s3m_mh->ordnum)) return 0;
	if(!(origpositions=(UWORD*)MikMod_calloc(s3m_mh->ordnum,sizeof(UWORD)))) return 0;

	for(t=0;t<s3m_mh->ordnum;t++) {
		origpositions[t]=_mm_read_UBYTE(modreader);
		if((origpositions[t]>=s3m_mh->patnum)&&(origpositions[t]<254))
			origpositions[t]=255/*s3m_mh->patnum-1*/;
	}
	...
	poslookupcnt= (UBYTE)s3m_mh->ordnum;
	S3MIT_CreateOrders(curious);

	if(!(s3m_paraptr=(UWORD*)MikMod_malloc((of.numins+of.numpat)*sizeof(UWORD))))
		return 0;

	/* read the instrument+pattern parapointers */
	_mm_read_I_UWORDS(s3m_paraptr,of.numins+of.numpat,modreader);

	if(s3m_mh->pantable==252) {
	...
	for(t=0;t<of.numins;t++) {
		...
		_mm_fseek(modreader,((long)s3m_paraptr[t])<<4,SEEK_SET);
		...
		if(s3m_mh->fileformat==1) q->flags |= SF_SIGNED;
	...
	for(t=0;t<of.numpat;t++) {
		...
		_mm_fseek(modreader,(long)((s3m_paraptr[of.numins+t])<<4)+2,SEEK_SET);
	...
	for(t=0;t<32;t++) 
		if((s3m_mh->channels[t]<32)&&(remap[t]!=-1)) {
			if(s3m_mh->channels[t]<8)
	...
	if(s3m_mh->pantable==252)
		...
		for(t=0;t<32;t++)
			if((pan[t]&0x20)&&(s3m_mh->channels[t]<32)&&(remap[t]!=-1))
