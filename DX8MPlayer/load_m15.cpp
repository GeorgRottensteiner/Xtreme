...
static BOOL LoadModuleHeader(tM15ModuleHeader *m15_mh)
{
	int t,u;

	_mm_read_string(m15_mh->songname,20,modreader);
	m15_mh->songname[20]=0;	/* just in case */

	/* sanity check : title should contain printable characters and a bunch
	   of null chars */
	for(t=0;t<20;t++)
		if((m15_mh->songname[t])&&(m15_mh->songname[t]<32)) return 0;
	for(t=0;(m15_mh->songname[t])&&(t<20);t++);
	if(t<20) for(;t<20;t++) if(m15_mh->songname[t]) return 0;

	for(t=0;t<15;t++) {
		tM15SampInfo *s=&m15_mh->samples[t];
	...
	m15_mh->songlength  =_mm_read_UBYTE(modreader);
	m15_mh->magic1      =_mm_read_UBYTE(modreader);	/* should be 127 */

	/* sanity check : no more than 128 positions, restart position in range */
	if((!m15_mh->songlength)||(m15_mh->songlength>128)) return 0;
	/* values encountered so far are 0x6a and 0x78 */
	if(((m15_mh->magic1&0xf8)!=0x78)&&(m15_mh->magic1!=0x6a)&&(m15_mh->magic1>m15_mh->songlength)) return 0;

	_mm_read_UBYTES(m15_mh->positions,128,modreader);

	/* sanity check : pattern range is 0..63 */
	for(t=0;t<128;t++)
		if(m15_mh->positions[t]>63) return 0;
...
static int CheckPatternType(int numpat)
{
	...
	for(t=0;t<numpat*(SLONG)(64U*4);t++) {
...
{
	int t, numpat;
	tM15ModuleHeader m15_mh;

	ust_loader = 0;
	if(!LoadModuleHeader(&m15_mh)) return 0;

	/* reject other file types */
	for(t=0;t<REJECT;t++)
		if(!memcmp(m15_mh.songname,signatures[t],siglen[t])) return 0;

	if(m15_mh.magic1>127) return 0;
	if((!m15_mh.songlength)||(m15_mh.songlength>m15_mh.magic1)) return 0;

	for(t=0;t<15;t++) {
		/* all finetunes should be zero */
		if(m15_mh.samples[t].finetune) return 0;

		/* all volumes should be <= 64 */
		if(m15_mh.samples[t].volume>64) return 0;

		/* all instrument names should begin with s, st-, or a number */
		if((m15_mh.samples[t].samplename[0]=='s')||
		   (m15_mh.samples[t].samplename[0]=='S')) {
			if((memcmp(m15_mh.samples[t].samplename,"st-",3)) &&
			   (memcmp(m15_mh.samples[t].samplename,"ST-",3)) &&
			   (*m15_mh.samples[t].samplename))
				ust_loader = 1;
		} else
		  if(!isdigit((int)m15_mh.samples[t].samplename[0]))
				ust_loader = 1;

		if(m15_mh.samples[t].length>4999||m15_mh.samples[t].reppos>9999) {
			ust_loader = 0;
			if(m15_mh.samples[t].length>32768) return 0;
		}

		/* if loop information is incorrect as words, but correct as bytes,
		   this is likely to be an ust-style module */
		if((m15_mh.samples[t].reppos+m15_mh.samples[t].replen>m15_mh.samples[t].length)&&
		   (m15_mh.samples[t].reppos+m15_mh.samples[t].replen<(m15_mh.samples[t].length<<1))){
			ust_loader = 1;
			return 1;
		}
	...
	for(numpat=0,t=0;t<m15_mh.songlength;t++) 
		if(m15_mh.positions[t]>numpat)
			numpat = m15_mh.positions[t];
	numpat++;
...
{
	if(!(m15_mh=(tM15ModuleHeader*)MikMod_malloc(sizeof(tM15ModuleHeader)))) return 0;
	return 1;
}
...
static UBYTE M15_ConvertNote(tM15ModNote* n, UBYTE lasteffect)
{
	...
	if(instrument) {
		...
		if((instrument>15)||(!m15_mh->samples[instrument-1].length)) {
			UniPTEffect(0xc,0);
			...
			/* ...otherwise, only adjust volume... */
				...
				if(effect||effdat) {
					...
					UniPTEffect(0xc,m15_mh->samples[instrument-1].volume&0x7f);
			}
		}
	}
	if(note) {
...
static UBYTE *M15_ConvertTrack(tM15ModNote* n)
{
	int t;
...
static BOOL M15_LoadPatterns(void)
{
	...
	if(!(m15patbuf=(tM15ModNote*)MikMod_calloc(64U*4,sizeof(tM15ModNote)))) return 0;

	for(t=0;t<of.numpat;t++) {
		/* Load the pattern into the temp buffer and convert it */
		for(s=0;s<(64U*4);s++) {
			m15patbuf[s].a=_mm_read_UBYTE(modreader);
			m15patbuf[s].b=_mm_read_UBYTE(modreader);
			m15patbuf[s].c=_mm_read_UBYTE(modreader);
			m15patbuf[s].d=_mm_read_UBYTE(modreader);
		}

		for(s=0;s<4;s++)
			if(!(of.tracks[tracks++]=M15_ConvertTrack(m15patbuf+s))) return 0;
	}
	return 1;
}
...
static BOOL M15_Load(BOOL curious)
{
	...
	if(!LoadModuleHeader(m15_mh)) {
		_mm_errno = MMERR_LOADING_HEADER;
		return 0;
	}

	if(ust_loader)
		of.modtype = _strdup("Ultimate Soundtracker");
	else
		of.modtype = _strdup("Soundtracker");

	/* set module variables */
	of.initspeed = 6;
	of.inittempo = 125;
	of.numchn    = 4;				
	of.songname  = DupStr(m15_mh->songname,21,1);
	of.numpos    = m15_mh->songlength;
	of.reppos    = 0;

	/* Count the number of patterns */
	of.numpat = 0;
	for(t=0;t<of.numpos;t++)
		if(m15_mh->positions[t]>of.numpat)
			of.numpat=m15_mh->positions[t];
	...
	for(t=of.numpos;t<128;t++)
		if(m15_mh->positions[t]>=0x80) scan=0;
	if (scan)
		for(t=of.numpos;t<128;t++) {
			if(m15_mh->positions[t]>of.numpat)
				of.numpat=m15_mh->positions[t];
			if((curious)&&(m15_mh->positions[t])) of.numpos=t+1;
		}
	of.numpat++;
	...
	for(t=0;t<of.numpos;t++)
		of.positions[t]=m15_mh->positions[t];
