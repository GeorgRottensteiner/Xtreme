...
BOOL XM_Init(void)
{
	if(!(xm_mh=(XMHEADER *)MikMod_malloc(sizeof(XMHEADER)))) return 0;
	return 1;
}
			...
			default:
				if(eff<=0xf) {
					/* the pattern jump destination is written in decimal,
					   ...
					   in hexadecimal... (sigh) */
					if (eff==0xd)
						/* don't change anything if we're sure it'xm_s in hexa */
...
static BOOL LoadPatterns(BOOL dummypat)
{
	...
	numtrk=0;
	for(t=0;t<xm_mh->numpat;t++) {
		XMPATHEADER ph;

		ph.size     =_mm_read_I_ULONG(modreader);
		if (ph.size< (ULONG)(xm_mh->version==0x0102?8:9)) {
		...
		if(ph.numrows) {
			...
			/* when packsize is 0, don't try to load a pattern.. it'xm_s empty. */
			if(ph.packsize) 
		...
		if((SWORD)ih.size>29) {
			...
			if(((SWORD)ih.numsmp>0)&&(ih.numsmp<=XMNOTECNT)) {
				...
				if((_mm_eof(modreader))||(pth.volpts>XMENVCNT/2)||(pth.panpts>XMENVCNT/2)) {
					...
					if(xm_wh) { MikMod_free(xm_wh);xm_wh=NULL; }
...
#undef XM_ProcessEnvelope
				...
				/* Samples are stored outside the instrument struct now, so we
				   ...
				   everything over */
				if(xm_mh->version>0x0103) next = 0;
				for(u=0;u<ih.numsmp;u++,xm_s++) {
					...
					if(of.numsmp+u==wavcnt) {
						...
						if(!(xm_wh=(XMWAVHEADER*)MikMod_realloc(xm_wh,wavcnt*sizeof(XMWAVHEADER)))) {
							MikMod_free(nextwav);nextwav=NULL;
							_mm_errno = MMERR_OUT_OF_MEMORY;
							return 0;
						}
						xm_s=xm_wh+(wavcnt-XM_SMPINCR);
					}

					xm_s->length       =_mm_read_I_ULONG (modreader);
					xm_s->loopstart    =_mm_read_I_ULONG (modreader);
					xm_s->looplength   =_mm_read_I_ULONG (modreader);
					xm_s->volume       =_mm_read_UBYTE (modreader);
					xm_s->finetune     =_mm_read_SBYTE (modreader);
					xm_s->type         =_mm_read_UBYTE (modreader);
					xm_s->panning      =_mm_read_UBYTE (modreader);
					xm_s->relnote      =_mm_read_SBYTE (modreader);
					xm_s->vibtype      = pth.vibflg;
					xm_s->vibsweep     = pth.vibsweep;
					xm_s->vibdepth     = pth.vibdepth*4;
					xm_s->vibrate      = pth.vibrate;
					xm_s->reserved     =_mm_read_UBYTE (modreader);
					_mm_read_string(xm_s->samplename, 22, modreader);

					nextwav[of.numsmp+u]=next;
					next+=xm_s->length;

					if(_mm_eof(modreader)) {
						MikMod_free(nextwav);MikMod_free(xm_wh);
						nextwav=NULL;xm_wh=NULL;
						_mm_errno = MMERR_LOADING_SAMPLEINFO;
						return 0;
					}
				}

				if(xm_mh->version>0x0103) {
...
BOOL XM_Load(BOOL curious)
{
	...
	_mm_read_string(xm_mh->id,17,modreader);
	_mm_read_string(xm_mh->songname,21,modreader);
	_mm_read_string(xm_mh->trackername,20,modreader);
	xm_mh->version     =_mm_read_I_UWORD(modreader);
	if((xm_mh->version<0x102)||(xm_mh->version>0x104)) {
		_mm_errno=MMERR_NOT_A_MODULE;
		return 0;
	}
	xm_mh->headersize  =_mm_read_I_ULONG(modreader);
	xm_mh->songlength  =_mm_read_I_UWORD(modreader);
	xm_mh->restart     =_mm_read_I_UWORD(modreader);
	xm_mh->numchn      =_mm_read_I_UWORD(modreader);
	xm_mh->numpat      =_mm_read_I_UWORD(modreader);
	xm_mh->numins      =_mm_read_I_UWORD(modreader);
	xm_mh->flags       =_mm_read_I_UWORD(modreader);
	xm_mh->tempo       =_mm_read_I_UWORD(modreader);
	xm_mh->bpm         =_mm_read_I_UWORD(modreader);
	if(!xm_mh->bpm) {
		_mm_errno=MMERR_NOT_A_MODULE;
		return 0;
	}
	_mm_read_UBYTES(xm_mh->orders,256,modreader);
	...
	of.initspeed = (UBYTE)xm_mh->tempo;         
	of.inittempo = xm_mh->bpm;
	strncpy(tracker,xm_mh->trackername,20);tracker[20]=0;
  ...
  /*
#ifdef HAVE_SNPRINTF
	snprintf(modtype,60,"%xm_s (XM format %d.%02d)",
	                    tracker,xm_mh->version>>8,xm_mh->version&0xff);
#else
	sprintf(modtype,"%xm_s (XM format %d.%02d)",
	                tracker,xm_mh->version>>8,xm_mh->version&0xff);
#endif
                  */

	of.modtype   = _strdup(modtype);
	of.numchn    = (UBYTE)xm_mh->numchn;
	of.numpat    = xm_mh->numpat;
	of.numtrk    = (UWORD)of.numpat*of.numchn;   /* get number of channels */
	of.songname  = DupStr(xm_mh->songname,20,1);
	of.numpos    = xm_mh->songlength;               /* copy the songlength */
	of.reppos    = xm_mh->restart<xm_mh->songlength?xm_mh->restart:0;
	of.numins    = xm_mh->numins;
	of.flags    |= UF_XMPERIODS | UF_INST | UF_NOWRAP | UF_FT2QUIRKS |
				   UF_PANNING;
	if(xm_mh->flags&1) of.flags |= UF_LINEAR;
	of.bpmlimit  = 32;
	...
	q = of.samples;
	xm_s = xm_wh;
	for(u=0;u<of.numsmp;u++,q++,xm_s++) {
		q->samplename   = DupStr(xm_s->samplename,22,1);
		q->length       = xm_s->length;
		q->loopstart    = xm_s->loopstart;
		q->loopend      = xm_s->loopstart+xm_s->looplength;
		q->volume       = xm_s->volume;
		q->speed        = xm_s->finetune+128;
		q->panning      = xm_s->panning;
		q->seekpos      = nextwav[u];
		q->vibtype      = xm_s->vibtype;
		q->vibsweep     = xm_s->vibsweep;
		q->vibdepth     = xm_s->vibdepth;
		q->vibrate      = xm_s->vibrate;
		...
		if(xm_s->type&0x3) q->flags|=SF_LOOP;
		if(xm_s->type&0x2) q->flags|=SF_BIDI;
		if(xm_s->type&0x10) q->flags|=SF_16BITS;
	}
	...
	for(u=0;u<of.numins;u++,d++)
		for(t=0;t<XMNOTECNT;t++) {
			if (d->samplenumber[t]>=of.numsmp)
				...
				int note=t+xm_s[d->samplenumber[t]].relnote;
				d->samplenote[t]=(note<0)?0:note;
			}
		}

	MikMod_free(xm_wh);MikMod_free(nextwav);
	xm_wh=NULL;nextwav=NULL;
	return 1;
}
...
CHAR *XM_LoadTitle(void)
{
	...
	if(!_mm_read_UBYTES(xm_s,21,modreader)) return NULL;

	return(DupStr(xm_s,21,1));
}
