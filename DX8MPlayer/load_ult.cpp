...
BOOL ULT_Load(BOOL curious)
{
	...
	_mm_read_string(ult_mh.id,15,modreader);
	_mm_read_string(ult_mh.songtitle,32,modreader);
	ult_mh.reserved=_mm_read_UBYTE(modreader);
	...
	ULT_Version[ULT_VERSION_LEN-1]='3'+(ult_mh.id[14]-'1');
	...
	/* read songtext */
	if ((ult_mh.id[14]>'1')&&(ult_mh.reserved))
		if(!ReadLinedComment(ult_mh.reserved * 32, 32)) return 0;
	...
	of.songname=DupStr(ult_mh.songtitle,32,1);
	...
	for(t=0;t<nos;t++) {
		...
		s.speed         =(ult_mh.id[14]>='4')?_mm_read_I_UWORD(modreader):8363;
