...
static STXHEADER *stx_mh      = NULL;
static UWORD     *stx_paraptr = NULL; /* parapointer array (see STX docs) */
...
static BOOL STX_Init(void)
{
	...
	if(!(stx_mh=(STXHEADER*)MikMod_malloc(sizeof(STXHEADER)))) return 0;
...
static BOOL STX_Load(BOOL curious)
{
	...
	_mm_read_string(stx_mh->songname,20,modreader);
	_mm_read_string(stx_mh->trackername,8,modreader);
	stx_mh->patsize     =_mm_read_I_UWORD(modreader);
	stx_mh->unknown1    =_mm_read_I_UWORD(modreader);
	stx_mh->patptr      =_mm_read_I_UWORD(modreader);
	stx_mh->insptr      =_mm_read_I_UWORD(modreader);
	stx_mh->chnptr      =_mm_read_I_UWORD(modreader);
	stx_mh->unknown2    =_mm_read_I_UWORD(modreader);
	stx_mh->unknown3    =_mm_read_I_UWORD(modreader);
	stx_mh->mastermult  =_mm_read_UBYTE(modreader);
	stx_mh->initspeed   =_mm_read_UBYTE(modreader)>>4;
	stx_mh->unknown4    =_mm_read_I_UWORD(modreader);
	stx_mh->unknown5    =_mm_read_I_UWORD(modreader);
	stx_mh->patnum      =_mm_read_I_UWORD(modreader);
	stx_mh->insnum      =_mm_read_I_UWORD(modreader);
	stx_mh->ordnum      =_mm_read_I_UWORD(modreader);
	stx_mh->unknown6    =_mm_read_I_UWORD(modreader);
	stx_mh->unknown7    =_mm_read_I_UWORD(modreader);
	stx_mh->unknown8    =_mm_read_I_UWORD(modreader);
	_mm_read_string(stx_mh->scrm,4,modreader);
	...
	of.songname    = DupStr(stx_mh->songname,20,1);
	of.numpat      = stx_mh->patnum;
	of.reppos      = 0;
	of.numins      = of.numsmp = stx_mh->insnum;
	of.initspeed   = stx_mh->initspeed;
	of.inittempo   = 125;
	of.numchn      = 4;
	of.flags      |= UF_S3MSLIDES;
	of.bpmlimit    = 32;

	if(!(stx_paraptr=(UWORD*)MikMod_malloc((of.numins+of.numpat)*sizeof(UWORD))))
		return 0;

	/* read the instrument+pattern parapointers */
	_mm_fseek(modreader,stx_mh->insptr<<4,SEEK_SET);
	_mm_read_I_UWORDS(stx_paraptr,of.numins,modreader);
	_mm_fseek(modreader,stx_mh->patptr<<4,SEEK_SET);
	_mm_read_I_UWORDS(stx_paraptr+of.numins,of.numpat,modreader);

	/* check module version */
	_mm_fseek(modreader,stx_paraptr[of.numins]<<4,SEEK_SET);
	version=_mm_read_I_UWORD(modreader);
	if(version==stx_mh->patsize) {
		version    = 0x10;
		of.modtype = _strdup("STMIK 0.2 (STM2STX 1.0)");
	} else {
		version    = 0x11;
		of.modtype = _strdup("STMIK 0.2 (STM2STX 1.1)");
	}

	/* read the order data */
	_mm_fseek(modreader,(stx_mh->chnptr<<4)+32,SEEK_SET);
	if(!AllocPositions(stx_mh->ordnum)) return 0;
	for(t=0;t<stx_mh->ordnum;t++) {
		of.positions[t]=_mm_read_UBYTE(modreader);
		_mm_fseek(modreader,4,SEEK_CUR);
	}

	of.numpos=0;
  poslookupcnt=(UBYTE)stx_mh->ordnum;
	for(t=0;t<stx_mh->ordnum;t++) {
		int order=of.positions[t];
		if(order==255) order=LAST_PATTERN;
		of.positions[of.numpos]=order;
		poslookup[t]= (UBYTE)of.numpos;	/* bug fix for freaky S3Ms */
	...
	for(q=of.samples,t=0;t<of.numins;t++,q++) {
		...
		_mm_fseek(modreader,((long)stx_paraptr[t])<<4,SEEK_SET);
	...
	for(t=0;t<of.numpat;t++) {
		...
		_mm_fseek(modreader,(((long)stx_paraptr[of.numins+t])<<4)+
