...
static MODULEHEADER *asy_mh = NULL;
...
{
	if (!(asy_mh = (MODULEHEADER *)MikMod_malloc(sizeof(MODULEHEADER))))
		return 0;
	return 1;
}
...
static void ConvertNote(MODNOTE *n)
{
	...
	if (instrument) {
		...
		if ((instrument > 31) || (!asy_mh->samples[instrument - 1].length)) {
			UniPTEffect(0xc, 0);
			if (effect == 0xc)
				effect = effdat = 0;
		} else {
...
static BOOL ASY_Load(BOOL curious)
{
	...
	strcpy(asy_mh->songname, "");

	_mm_fseek(modreader, 0x23, SEEK_SET);
	asy_mh->num_patterns = _mm_read_UBYTE(modreader);
	asy_mh->num_orders = _mm_read_UBYTE(modreader);
	
	// skip unknown byte
	_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(asy_mh->positions, 256, modreader);
	...
	of.numchn = 8;
	modtype = 0;
	of.songname = DupStr(asy_mh->songname, 21, 1);
	of.numpos = asy_mh->num_orders;
	of.reppos = 0;
	of.numpat = asy_mh->num_patterns;
	...
	for (t = 0; t < of.numpos; t++) {
		of.positions[t] = asy_mh->positions[t];
	}
