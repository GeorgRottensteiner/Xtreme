...
BOOL IT_Init(void)
{
	if(!(it_mh=(ITHEADER*)MikMod_malloc(sizeof(ITHEADER)))) return 0;
...
BOOL IT_Load(BOOL curious)
{
	...
	_mm_read_string(it_mh->songname,26,modreader);
	_mm_read_UBYTES(it_mh->blank01,2,modreader);
	it_mh->ordnum      =_mm_read_I_UWORD(modreader);
	it_mh->insnum      =_mm_read_I_UWORD(modreader);
	it_mh->smpnum      =_mm_read_I_UWORD(modreader);
	it_mh->patnum      =_mm_read_I_UWORD(modreader);
	it_mh->cwt         =_mm_read_I_UWORD(modreader);
	it_mh->cmwt        =_mm_read_I_UWORD(modreader);
	it_mh->flags       =_mm_read_I_UWORD(modreader);
	it_mh->special     =_mm_read_I_UWORD(modreader);
	it_mh->globvol     =_mm_read_UBYTE(modreader);
	it_mh->mixvol      =_mm_read_UBYTE(modreader);
	it_mh->initspeed   =_mm_read_UBYTE(modreader);
	it_mh->inittempo   =_mm_read_UBYTE(modreader);
	it_mh->pansep      =_mm_read_UBYTE(modreader);
	it_mh->zerobyte    =_mm_read_UBYTE(modreader);
	it_mh->msglength   =_mm_read_I_UWORD(modreader);
	it_mh->msgoffset   =_mm_read_I_ULONG(modreader);
	_mm_read_UBYTES(it_mh->blank02,4,modreader);
	_mm_read_UBYTES(it_mh->pantable,64,modreader);
	_mm_read_UBYTES(it_mh->voltable,64,modreader);
	...
	of.songname    = DupStr(it_mh->songname,26,0); /* make a cstr of songname  */
	of.reppos      = 0;
	of.numpat      = it_mh->patnum;
	of.numins      = it_mh->insnum;
	of.numsmp      = it_mh->smpnum;
	of.initspeed   = it_mh->initspeed;
	of.inittempo   = it_mh->inittempo;
	of.initvolume  = it_mh->globvol;
	of.flags      |= UF_BGSLIDES | UF_ARPMEM;
	if (!(it_mh->flags & 1))
			of.flags |= UF_PANNING;
	of.bpmlimit=32;

	if(it_mh->songname[25]) {
		of.numvoices=1+it_mh->songname[25];
#ifdef MIKMOD_DEBUG
	...
	if((it_mh->cwt<=0x219)&&(it_mh->cwt>=0x217))
		of.modtype=_strdup(IT_Version[it_mh->cmwt<0x214?4:5]);
	else if (it_mh->cwt>=0x215)
		of.modtype=_strdup(IT_Version[it_mh->cmwt<0x214?2:3]);
	else {
		of.modtype     = _strdup(IT_Version[it_mh->cmwt<0x214?0:1]);
		of.modtype[it_mh->cmwt<0x214?15:26] = (it_mh->cwt>>8)+'0';
		of.modtype[it_mh->cmwt<0x214?17:28] = ((it_mh->cwt>>4)&0xf)+'0';
		of.modtype[it_mh->cmwt<0x214?18:29] = ((it_mh->cwt)&0xf)+'0';
	}

	if(it_mh->flags&8)
		of.flags |= UF_XMPERIODS | UF_LINEAR;

	if((it_mh->cwt>=0x106)&&(it_mh->flags&16))
	...
	if (it_mh->flags & 1)
		for(t=0;t<64;t++) {
			it_mh->pantable[t]&=0x7f;
			if(it_mh->pantable[t]<64)
				of.panning[t]=it_mh->pantable[t]<<2;
			else if(it_mh->pantable[t]==64)
				of.panning[t]=255;
			else if(it_mh->pantable[t]==100)
				of.panning[t]=PAN_SURROUND;
			else if(it_mh->pantable[t]==127)
	...
	/* set channel volumes */
	memcpy(of.chanvol,it_mh->voltable,64);

	/* read the order data */
	if(!AllocPositions(it_mh->ordnum)) return 0;
	if(!(origpositions=(UWORD*)MikMod_calloc(it_mh->ordnum,sizeof(UWORD)))) return 0;

	for(t=0;t<it_mh->ordnum;t++) {
		origpositions[t]=_mm_read_UBYTE(modreader);
		if((origpositions[t]>it_mh->patnum)&&(origpositions[t]<254))
			origpositions[t]=255;
	}
	...
	poslookupcnt=(UBYTE)it_mh->ordnum;
	S3MIT_CreateOrders(curious);

	if(!(paraptr=(ULONG*)MikMod_malloc((it_mh->insnum+it_mh->smpnum+of.numpat)*
	                               sizeof(ULONG)))) return 0;

	/* read the instrument, sample, and pattern parapointers */
	_mm_read_I_ULONGS(paraptr,it_mh->insnum+it_mh->smpnum+of.numpat,modreader);
	...
	if((it_mh->special&1)&&(it_mh->cwt>=0x104)&&(it_mh->msglength)) {
		_mm_fseek(modreader,(long)(it_mh->msgoffset),SEEK_SET);
		if(!ReadComment(it_mh->msglength)) return 0;
	}

	if(!(it_mh->flags&4)) of.numins=of.numsmp;
	...
	/* Load all samples */
	q = of.samples;
	for(t=0;t<it_mh->smpnum;t++) {
		ITSAMPLE s;

		/* seek to sample position */
		_mm_fseek(modreader,(long)(paraptr[it_mh->insnum+t]+4),SEEK_SET);
...
#undef IT_ProcessEnvelope
	...
	for(t=0;t<of.numpat;t++) {
		...
		if(paraptr[it_mh->insnum+it_mh->smpnum+t]) { /* 0 -> empty 64 row pattern */
			_mm_fseek(modreader,((long)paraptr[it_mh->insnum+it_mh->smpnum+t]),SEEK_SET);
			_mm_read_I_UWORD(modreader);
