...
BOOL MED_Init(void)
{
	if (!(me = (MEDEXP *)MikMod_malloc(sizeof(MEDEXP))))
		return 0;
	if (!(med_mh = (MEDHEADER *)MikMod_malloc(sizeof(MEDHEADER))))
		return 0;
	  ...
	  case 0xf:
		switch (dat) {
		  ...
		  case 0xfe:			/* stop playing */
			UniPTEffect(0xb, (UBYTE)of.numpat);
			break;
		  case 0xff:			/* note cut */
			UniPTEffect(0xc, 0);
			break;
...
BOOL MED_Load(BOOL curious)
{
	...
	med_mh->id = _mm_read_M_ULONG(modreader);
	med_mh->modlen = _mm_read_M_ULONG(modreader);
	med_mh->MEDSONGP = _mm_read_M_ULONG(modreader);
	med_mh->psecnum = _mm_read_M_UWORD(modreader);
	med_mh->pseq = _mm_read_M_UWORD(modreader);
	med_mh->MEDBlockPP = _mm_read_M_ULONG(modreader);
	med_mh->reserved1 = _mm_read_M_ULONG(modreader);
	med_mh->MEDINSTHEADERPP = _mm_read_M_ULONG(modreader);
	med_mh->reserved2 = _mm_read_M_ULONG(modreader);
	med_mh->MEDEXPP = _mm_read_M_ULONG(modreader);
	med_mh->reserved3 = _mm_read_M_ULONG(modreader);
	med_mh->pstate = _mm_read_M_UWORD(modreader);
	med_mh->pblock = _mm_read_M_UWORD(modreader);
	med_mh->pline = _mm_read_M_UWORD(modreader);
	med_mh->pseqnum = _mm_read_M_UWORD(modreader);
	med_mh->actplayline = _mm_read_M_SWORD(modreader);
	med_mh->counter = _mm_read_UBYTE(modreader);
	med_mh->extra_songs = _mm_read_UBYTE(modreader);

	/* Seek to MEDSONG struct */
	_mm_fseek(modreader, med_mh->MEDSONGP, SEEK_SET);
	...
	if (med_mh->MEDEXPP) {
		_mm_fseek(modreader, med_mh->MEDEXPP, SEEK_SET);
	...
	_mm_fseek(modreader, med_mh->MEDINSTHEADERPP, SEEK_SET);
	...
	if (!(ba = (ULONG *)MikMod_calloc(ms->numblocks, sizeof(ULONG))))
		return 0;
	_mm_fseek(modreader, med_mh->MEDBlockPP, SEEK_SET);
	...
	MED_Version[12] = (CHAR)med_mh->id;
	of.modtype = _strdup(MED_Version);
	...
	of.numsmp = of.numins;
	of.reppos = 0;
	if ((med_mh->MEDEXPP) && (me->songname) && (me->songnamelen)) {
		char *name;
		...
		of.songname = DupStr(name, (UWORD)me->songnamelen, 1);
		MikMod_free(name);
	} else
		of.songname = DupStr(NULL, 0, 0);
	if ((med_mh->MEDEXPP) && (me->annotxt) && (me->annolen)) {
		_mm_fseek(modreader, me->annotxt, SEEK_SET);
		ReadComment( (UWORD)me->annolen);
	}
	...
	if (med_mh->id == MMD0_string) {
		if (!LoadMEDPatterns()) {
			_mm_errno = MMERR_LOADING_PATTERN;
			return 0;
		}
	} else if (med_mh->id == MMD1_string) {
		if (!LoadMMD1Patterns()) {
			_mm_errno = MMERR_LOADING_PATTERN;
			return 0;
		}
...
CHAR *MED_LoadTitle(void)
{
	...
	if (posit) {
		...
		retvalue = DupStr(name, (UWORD)namelen, 1);
		MikMod_free(name);
	}
