...
BOOL DSM_Init(void)
{
	...
	if(!(dsm_mh=(DSMSONG *)MikMod_calloc(1,sizeof(DSMSONG)))) return 0;
	return 1;
}
...
BOOL DSM_Load(BOOL curious)
{
	...
	_mm_read_UBYTES(dsm_mh->songname,28,modreader);
	dsm_mh->version=_mm_read_I_UWORD(modreader);
	dsm_mh->flags=_mm_read_I_UWORD(modreader);
	dsm_mh->reserved2=_mm_read_I_ULONG(modreader);
	dsm_mh->numord=_mm_read_I_UWORD(modreader);
	dsm_mh->numsmp=_mm_read_I_UWORD(modreader);
	dsm_mh->numpat=_mm_read_I_UWORD(modreader);
	dsm_mh->numtrk=_mm_read_I_UWORD(modreader);
	dsm_mh->globalvol=_mm_read_UBYTE(modreader);
	dsm_mh->mastervol=_mm_read_UBYTE(modreader);
	dsm_mh->speed=_mm_read_UBYTE(modreader);
	dsm_mh->bpm=_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(dsm_mh->panpos,DSM_MAXCHAN,modreader);
	_mm_read_UBYTES(dsm_mh->orders,DSM_MAXORDERS,modreader);

	/* set module variables */
	of.initspeed=dsm_mh->speed;
	of.inittempo=dsm_mh->bpm;
	of.modtype=_strdup(DSM_Version);
	of.numchn=(UBYTE)dsm_mh->numtrk;
	of.numpat=dsm_mh->numpat;
	of.numtrk=of.numchn*of.numpat;
	of.songname=DupStr(dsm_mh->songname,28,1); /* make a cstr of songname */
	of.reppos=0;
	of.flags |= UF_PANNING;
	...
	for(t=0;t<DSM_MAXCHAN;t++)
		of.panning[t]=dsm_mh->panpos[t]==DSM_SURROUND?PAN_SURROUND:
		              dsm_mh->panpos[t]<0x80?(dsm_mh->panpos[t]<<1):255;

	if(!AllocPositions(dsm_mh->numord)) return 0;
	of.numpos=0;
	for(t=0;t<dsm_mh->numord;t++) {
		int order=dsm_mh->orders[t];
		if(order==255) order=LAST_PATTERN;
		of.positions[of.numpos]=order;
		if(dsm_mh->orders[t]<254) of.numpos++;
	}

	of.numins=of.numsmp=dsm_mh->numsmp;
