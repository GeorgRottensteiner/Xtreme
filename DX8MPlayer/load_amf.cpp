...
BOOL AMF_Init(void)
{
	if(!(amf_mh=(AMFHEADER*)MikMod_malloc(sizeof(AMFHEADER)))) return 0;
...
BOOL AMF_Load(BOOL curious)
{
	...
	_mm_read_UBYTES(amf_mh->id,3,modreader);
	amf_mh->version     =_mm_read_UBYTE(modreader);
	_mm_read_string(amf_mh->songname,32,modreader);
	amf_mh->numsamples  =_mm_read_UBYTE(modreader);
	amf_mh->numorders   =_mm_read_UBYTE(modreader);
	amf_mh->numtracks   =_mm_read_I_UWORD(modreader);
	amf_mh->numchannels =_mm_read_UBYTE(modreader);
	if((!amf_mh->numchannels)||(amf_mh->numchannels>(amf_mh->version>=12?32:16))) {
		_mm_errno=MMERR_NOT_A_MODULE;
		return 0;
	}

	if(amf_mh->version>=11) {
		memset(amf_mh->panpos,0,32);
		_mm_read_SBYTES(amf_mh->panpos,(amf_mh->version>=13)?32:16,modreader);
	} else
		_mm_read_UBYTES(channel_remap,16,modreader);

	if (amf_mh->version>=13) {
		amf_mh->songbpm=_mm_read_UBYTE(modreader);
		if(amf_mh->songbpm<32) {
			_mm_errno=MMERR_NOT_A_MODULE;
			return 0;
		}
		amf_mh->songspd=_mm_read_UBYTE(modreader);
		if(amf_mh->songspd>32) {
			_mm_errno=MMERR_NOT_A_MODULE;
			return 0;
		}
	} else {
	...
	of.initspeed = amf_mh->songspd;
	of.inittempo = amf_mh->songbpm;
	AMF_Version[AMFTEXTLEN-3]='0'+(amf_mh->version/10);
	AMF_Version[AMFTEXTLEN-1]='0'+(amf_mh->version%10);
	of.modtype   = _strdup(AMF_Version);
	of.numchn    = amf_mh->numchannels;
	of.numtrk    = amf_mh->numorders*amf_mh->numchannels;
	if (amf_mh->numtracks>of.numtrk)
		of.numtrk=amf_mh->numtracks;
	of.numtrk++;	/* add room for extra, empty track */
	of.songname  = DupStr(amf_mh->songname,32,1);
	of.numpos    = amf_mh->numorders;
	of.numpat    = amf_mh->numorders;
	of.reppos    = 0;
	...
	for (t = 0; t < 32; t++) {
		if (amf_mh->panpos[t] > 64) {
			...
			if (amf_mh->panpos[t] == 64)
				...
				of.panning[t] = (amf_mh->panpos[t] + 64) << 1;
	}
	if (defaultpanning) {
	...
	of.numins=of.numsmp=amf_mh->numsamples;
	...
	for (t=0;t<of.numpat;t++) {
		if (amf_mh->version>=14)
			/* track size */
			of.pattrows[t]=_mm_read_I_UWORD(modreader);
		if (amf_mh->version>=10)
	...
	/* read track table */
	if(!(track_remap=(UWORD*)MikMod_calloc(amf_mh->numtracks+1,sizeof(UWORD))))
		return 0;
	_mm_read_I_UWORDS(track_remap+1,amf_mh->numtracks,modreader);
	if(_mm_eof(modreader)) {
	...
	for( realsmpcnt=t=0;t<of.numsmp;t++)
		if( (ULONG)realsmpcnt<of.samples[t].seekpos)
