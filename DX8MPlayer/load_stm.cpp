...
static STMHEADER *stm_mh = NULL;

/* tracker identifiers */
/*
externstatic CHAR* STM_Version[STM_NTRACKERS] = {
	"Screamtracker 2",
...
BOOL STM_Init(void)
{
	if(!(stm_mh=(STMHEADER*)MikMod_malloc(sizeof(STMHEADER)))) return 0;
...
BOOL STM_Load(BOOL curious)
{
	...
	_mm_read_string(stm_mh->songname,20,modreader);
	_mm_read_string(stm_mh->trackername,8,modreader);
	stm_mh->unused      =_mm_read_UBYTE(modreader);
	stm_mh->filetype    =_mm_read_UBYTE(modreader);
	stm_mh->ver_major   =_mm_read_UBYTE(modreader);
	stm_mh->ver_minor   =_mm_read_UBYTE(modreader);
	stm_mh->inittempo   =_mm_read_UBYTE(modreader);
	if(!stm_mh->inittempo) {
		_mm_errno=MMERR_NOT_A_MODULE;
		return 0;
	}
	stm_mh->numpat      =_mm_read_UBYTE(modreader);
	stm_mh->globalvol   =_mm_read_UBYTE(modreader);
	_mm_read_UBYTES(stm_mh->reserved,13,modreader);

	for(t=0;t<31;t++) {
		STMSAMPLE *s=&stm_mh->sample[t];	/* STM sample data */
	...
	_mm_read_UBYTES(stm_mh->patorder,128,modreader);
	...
	for(t=0;t<STM_NTRACKERS;t++)
		if(!memcmp(stm_mh->trackername,STM_Signatures[t],8)) break;
	of.modtype   = _strdup(STM_Version[t]);
	of.songname  = DupStr(stm_mh->songname,20,1); /* make a cstr of songname */
	of.numpat    = stm_mh->numpat;
	of.inittempo = 125;                     /* stm_mh->inittempo+0x1c; */
	of.initspeed = stm_mh->inittempo>>4;
	...
	while((stm_mh->patorder[t]<=99)&&(stm_mh->patorder[t]<stm_mh->numpat)) {
		of.positions[t]=stm_mh->patorder[t];
		t++;
	}
	if(stm_mh->patorder[t]<=99) t++;
	of.numpos=t;
	...
	for(q=of.samples,t=0;t<of.numsmp;t++,q++) {
		/* load sample info */
		q->samplename = DupStr(stm_mh->sample[t].filename,12,1);
		q->speed      = (stm_mh->sample[t].c2spd * 8363) / 8448;
		q->volume     = stm_mh->sample[t].volume;
		q->length     = stm_mh->sample[t].length;
		if (/*(!stm_mh->sample[t].volume)||*/(q->length==1)) q->length=0;
		q->loopstart  = stm_mh->sample[t].loopbeg;
		q->loopend    = stm_mh->sample[t].loopend;
