...
BOOL UNI_Load(BOOL curious)
{
	...
	/* read module header */
	_mm_read_UBYTES(uni_mh.id,4,modreader);
	if(uni_mh.id[3]!='N')
		universion=uni_mh.id[3]-'0';
	else
		universion=0x100;
	...
	if(universion>=6) {
		...
		uni_mh.flags     =_mm_read_M_UWORD(modreader);
		uni_mh.numchn    =_mm_read_UBYTE(modreader);
		uni_mh.numvoices =_mm_read_UBYTE(modreader);
		uni_mh.numpos    =_mm_read_M_UWORD(modreader);
		uni_mh.numpat    =_mm_read_M_UWORD(modreader);
		uni_mh.numtrk    =_mm_read_M_UWORD(modreader);
		uni_mh.numins    =_mm_read_M_UWORD(modreader);
		uni_mh.numsmp    =_mm_read_M_UWORD(modreader);
		uni_mh.reppos    =_mm_read_M_UWORD(modreader);
		uni_mh.initspeed =_mm_read_UBYTE(modreader);
		uni_mh.inittempo =_mm_read_UBYTE(modreader);
		uni_mh.initvolume=_mm_read_UBYTE(modreader);
		/* I expect this to show up soon in APlayer 1.06 format */
		if (universion >= 0x106)
			uni_mh.bpmlimit=_mm_read_M_UWORD(modreader);
		else
			uni_mh.bpmlimit=32;

		uni_mh.flags &= UF_XMPERIODS | UF_LINEAR | UF_INST | UF_NNA;
		uni_mh.flags |= UF_PANNING;
	} else {
		uni_mh.numchn    =_mm_read_UBYTE(modreader);
		uni_mh.numpos    =_mm_read_I_UWORD(modreader);
		uni_mh.reppos    =(universion==5)?_mm_read_I_UWORD(modreader):0;
		uni_mh.numpat    =_mm_read_I_UWORD(modreader);
		uni_mh.numtrk    =_mm_read_I_UWORD(modreader);
		uni_mh.numins    =_mm_read_I_UWORD(modreader);
		uni_mh.initspeed =_mm_read_UBYTE(modreader);
		uni_mh.inittempo =_mm_read_UBYTE(modreader);
		_mm_read_UBYTES(uni_mh.positions,256,modreader);
		_mm_read_UBYTES(uni_mh.panning,32,modreader);
		uni_mh.flags     =_mm_read_UBYTE(modreader);
		uni_mh.bpmlimit  =32;

		uni_mh.flags &= UF_XMPERIODS | UF_LINEAR;
		uni_mh.flags |= UF_INST | UF_NOWRAP | UF_PANNING;
	}
	
	/* set module parameters */
	of.flags     =uni_mh.flags;
	of.numchn    =uni_mh.numchn;
	of.numpos    =uni_mh.numpos;
	of.numpat    =uni_mh.numpat;
	of.numtrk    =uni_mh.numtrk;
	of.numins    =uni_mh.numins;
	of.reppos    =uni_mh.reppos;
	of.initspeed =uni_mh.initspeed;
	of.inittempo =uni_mh.inittempo;
	if(uni_mh.bpmlimit)
		of.bpmlimit=uni_mh.bpmlimit;
	else
	...
	if(universion>=6) {
		of.numvoices=uni_mh.numvoices;
		of.initvolume=uni_mh.initvolume;
	}
	...
	if(universion>=6) {
		...
		if((uni_mh.numpos>256)||(uni_mh.numchn>32)) {
			_mm_errno=MMERR_LOADING_HEADER;
			return 0;
		}
		for(t=0;t<of.numpos;t++) of.positions[t]=uni_mh.positions[t];
		for(t=0;t<of.numchn;t++) of.panning[t]=uni_mh.panning[t];
	}
